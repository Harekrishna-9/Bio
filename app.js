...
async function loadAll(){
  ...
  await Promise.all([loadSettings(), loadStories(), loadHighlights(), loadGallery(), loadCounts()]);
  await registerView();
}
...
$("#musicBtn").onclick=()=>{
  const a=$("#audio");
  if(!a.src)return toast("Music add nahi hai");
  if(a.paused){
    a.play().then(()=>{$("#musicBtn").textContent="❚❚";$("#musicState").textContent="Playing now"}).catch(()=>toast("Audio play nahi hua"));
  }else{
    a.pause(); $("#musicBtn").textContent="▶"; $("#musicState").textContent="Paused";
  }
};
$("#audio").onended=()=>{$("#musicBtn").textContent="▶";$("#musicState").textContent="Finished"};
...
function showStory(){
  ...
  $("#storyTime").textContent="24h story";
