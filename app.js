...
async function loadSettings(){
  ...
  if(!error && data){
    ...
    if(heroName && heroName.firstChild){
      heroName.firstChild.nodeValue=(data.display_name||"Harekrishna Patel")+" ";
    }
    const heroVerify=document.querySelector(".hero h1 .verify");
    if(heroVerify){
      heroVerify.style.display=data.verified===false?"none":"inline-grid";
    }
    const heroTagline=document.querySelector(".hero .tagline");
    if(heroTagline){
      heroTagline.textContent=data.profession||"Web Creator • Tech Enthusiast • Inventory Professional";
    }
    const heroLocation=document.querySelector(".hero .meta-row span:first-child");
    if(heroLocation){
      heroLocation.textContent="📍 "+(data.location_text||"India");
    }
    const heroPhoto=document.querySelector(".hero img.avatar");
    if(heroPhoto && data.profile_photo_url){
      heroPhoto.src=data.profile_photo_url;
    }
