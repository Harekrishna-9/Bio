...
window.updateHighlight=async(id,name,emoji)=>{const n=prompt("Highlight name:",name);if(n===null)return;const e=prompt("Emoji:",emoji);if(e===null)return;const {error}=await sb.from("highlights").update({name:n.trim()||name,emoji:e.trim()||emoji}).eq("id",id);if(error)return alert(error.message);loadHighlights()}
window.deleteHighlight=async id=>{if(!confirm("Delete Highlight and all items?"))return;const {error}=await sb.from("highlights").delete().eq("id",id);if(error)return alert(error.message);loadHighlights()}
...
async function loadGallery(){const {data,error}=await sb.from("gallery").select("*").order("created_at",{ascending:false});if(error){$("#galleryList").innerHTML=`<div class="notice-box">${esc(error.message)}</div>`;return}$("#galleryList").innerHTML=(data||[]).map(g=>`<div class="gallery-tile"><img src="${esc(g.image_url)}"><button onclick="deleteGallery('${g.id}')">×</button><small>${esc((g.album_name||"Memories")+" • "+(g.caption||"No caption"))}</small><div class="tile-edit" onclick="updateGallery('${g.id}','${esc(g.album_name||"Memories").replace(/'/g,"&#39;")}','${esc(g.caption||"").replace(/'/g,"&#39;")}')">Edit</div></div>`).join("")||'<div class="notice-box">No gallery images.</div>'}
...
window.updateGallery=async(id,album,caption)=>{const a=prompt("Album name:",album);if(a===null)return;const c=prompt("Caption:",caption);if(c===null)return;const {error}=await sb.from("gallery").update({album_name:a.trim()||"Memories",caption:c.trim()||null}).eq("id",id);if(error)return alert(error.message);loadGallery()}
window.deleteGallery=async id=>{if(!confirm("Delete gallery item?"))return;const {error}=await sb.from("gallery").delete().eq("id",id);if(error)return alert(error.message);loadGallery()}
