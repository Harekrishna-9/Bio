...
function enforceLoggedOutViewV6(){
  document.body.classList.remove("admin-mode");
  const adminEl=$("#admin"), loginEl=$("#loginBox");
  if(adminEl)adminEl.classList.add("hide");
  if(loginEl)loginEl.classList.remove("hide");
}
enforceLoggedOutViewV6();
...
async function showAdmin(){document.body.classList.add("admin-mode");$("#loginBox").classList.add("hide");$("#admin").classList.remove("hide");applyDashboardLayoutV6();await refreshAll()}
...
$("#saveProfileSettings").onclick=async()=>{
  try{
    ...
    const {data,error}=await sb.rpc("admin_save_profile_v5",{
      p_note:$("#note").value.trim()||null,
      p_profile_status:$("#profileStatus").value||"Available",
      p_announcement_text:$("#announcementText").value.trim()||null,
      p_announcement_enabled:$("#announcementEnabled").checked,
      p_music_title:settingsData?.music_title||"Favourite Music",
      p_music_url:settingsData?.music_url||null,
      p_display_name:$("#displayName").value.trim()||"Harekrishna Patel",
      p_profession:$("#profession").value.trim()||"Web Creator • Tech Enthusiast • Inventory Professional",
      p_location_text:$("#locationText").value.trim()||"India",
      p_profile_photo_url:profile_photo_url,
      p_verified:$("#verifiedBadge").value==="true"
    });

    if(error)throw error;
    settingsData=data||settingsData;
    settingsId=data?.id||settingsId;
    if($("#profilePhotoFile"))$("#profilePhotoFile").value="";
    setMsg("#profileMsg","Profile settings saved ✅");
    await loadProfileSettings();
  }catch(e){
    setMsg("#profileMsg","Save failed: "+e.message,true);
  }
};
...
\n\n/* ===== Dashboard V6 layout controls (UI only; existing functions untouched) ===== */\nconst DASHBOARD_LAYOUT_KEY_V6="hk_bio_admin_v6_layout";\nconst DEFAULT_DASHBOARD_ORDER_V6=["profile","music","stories","archive","highlights","gallery","links","messages","analytics","linkstats"];\nfunction dashboardSectionsV6(){return [...document.querySelectorAll("#admin [data-dashboard-section]")]}\nfunction readDashboardLayoutV6(){\n  try{const x=JSON.parse(localStorage.getItem(DASHBOARD_LAYOUT_KEY_V6)||"null");if(x&&Array.isArray(x.order)&&x.hidden)return x}catch(e){}\n  return {order:[...DEFAULT_DASHBOARD_ORDER_V6],hidden:{}};\n}\nfunction applyDashboardLayoutV6(){\n  const admin=$("#admin"); if(!admin)return;\n  const layout=readDashboardLayoutV6();\n  const map=Object.fromEntries(dashboardSectionsV6().map(el=>[el.dataset.dashboardSection,el]));\n  const anchor=document.querySelector("#admin .quick-nav");\n  let insertAfter=anchor;\n  layout.order.forEach(key=>{\n    const el=map[key]; if(!el)return;\n    if(insertAfter&&insertAfter.parentNode===admin){insertAfter.insertAdjacentElement("afterend",el);insertAfter=el}\n    el.classList.toggle("section-hidden-v6",!!layout.hidden[key]);\n  });\n  renderDashboardCustomizeV6();\n}\nfunction renderDashboardCustomizeV6(){\n  const list=$("#dashboardSectionList"); if(!list)return;\n  const layout=readDashboardLayoutV6();\n  const map=Object.fromEntries(dashboardSectionsV6().map(el=>[el.dataset.dashboardSection,el]));\n  const order=[...layout.order,...Object.keys(map).filter(k=>!layout.order.includes(k))];\n  list.innerHTML=order.filter(k=>map[k]).map((key,i)=>{const hidden=!!layout.hidden[key];return `<div class="customize-item" draggable="true" data-key="${key}"><span class="drag">⋮⋮</span><b>${esc(map[key].dataset.title||key)}</b><button type="button" onclick="moveDashboardSectionV6('${key}',-1)">↑</button><button type="button" onclick="moveDashboardSectionV6('${key}',1)">↓</button><button type="button" class="visibility ${hidden?'off':''}" onclick="toggleDashboardSectionV6('${key}')">${hidden?'Show':'Hide'}</button></div>`}).join("");\n  enableDashboardDragV6();\n}\nwindow.moveDashboardSectionV6=(key,dir)=>{const l=readDashboardLayoutV6(),i=l.order.indexOf(key),j=i+dir;if(i<0||j<0||j>=l.order.length)return;[l.order[i],l.order[j]]=[l.order[j],l.order[i]];localStorage.setItem(DASHBOARD_LAYOUT_KEY_V6,JSON.stringify(l));renderDashboardCustomizeV6()};\nwindow.toggleDashboardSectionV6=key=>{const l=readDashboardLayoutV6();l.hidden[key]=!l.hidden[key];localStorage.setItem(DASHBOARD_LAYOUT_KEY_V6,JSON.stringify(l));renderDashboardCustomizeV6()};\nfunction enableDashboardDragV6(){const list=$("#dashboardSectionList");if(!list)return;let d=null;list.querySelectorAll(".customize-item").forEach(item=>{item.addEventListener("dragstart",()=>{d=item;item.style.opacity=".5"});item.addEventListener("dragend",()=>{item.style.opacity="";d=null;const l=readDashboardLayoutV6();l.order=[...list.querySelectorAll(".customize-item")].map(x=>x.dataset.key);localStorage.setItem(DASHBOARD_LAYOUT_KEY_V6,JSON.stringify(l))});item.addEventListener("dragover",e=>{e.preventDefault();if(!d||d===item)return;const r=item.getBoundingClientRect();list.insertBefore(d,e.clientY<r.top+r.height/2?item:item.nextSibling)})})}\nfunction openDashboardCustomizeV6(){renderDashboardCustomizeV6();$("#dashboardCustomizeModal")?.classList.add("open")}\n$("#customizeDashboardBtn")?.addEventListener("click",openDashboardCustomizeV6);\n$("#sidebarCustomize")?.addEventListener("click",openDashboardCustomizeV6);\n$("#closeCustomizeDashboard")?.addEventListener("click",()=>$("#dashboardCustomizeModal")?.classList.remove("open"));\n$("#saveDashboardLayout")?.addEventListener("click",()=>{applyDashboardLayoutV6();$("#dashboardCustomizeModal")?.classList.remove("open")});\n$("#resetDashboardLayout")?.addEventListener("click",()=>{localStorage.removeItem(DASHBOARD_LAYOUT_KEY_V6);applyDashboardLayoutV6()});\ndocument.querySelectorAll("[data-v6-nav]").forEach(b=>b.addEventListener("click",()=>{const k=b.dataset.v6Nav;if(k==="top"){window.scrollTo({top:0,behavior:"smooth"})}else{const el=document.querySelector(`#admin [data-dashboard-section="${k}"]`);if(el&&!el.classList.contains("section-hidden-v6"))el.scrollIntoView({behavior:"smooth",block:"start"})}$("#adminSidebar")?.classList.remove("open")}));\n$("#mobileSidebarToggle")?.addEventListener("click",()=>$("#adminSidebar")?.classList.toggle("open"));\n